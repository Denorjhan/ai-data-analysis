...
if st.session_state.uploaded_files:
    ...
    if st.button("Add to Data Catalog"):  #! add warning on time and cost
        try:
                # convert to parquet
                # upload to s3
            # run crawler to add to glue data catalog
            # create ddl
            # train model
            with st.status("Adding to Data Catalog...", expanded=True) as status:
                ...
                for file in st.session_state.uploaded_files:
                    ...
                    upload_to_s3(parquet_buffer, config['aws']['s3']['bucket'], parquet_file_key) 
                    i += 1
                ...
                create_glue_crawler(crawler_name, config['aws']['glue']['database'], s3_target_path)
