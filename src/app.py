...
def new_convo():
    st.session_state.messages = [
        {
            "role": "assistant",
            "content": "Ask me a question about your data to get started!",
            "avatar": ai_icon,
            "content_type": "text",
        }
    ]
...
st.sidebar.checkbox(
    "Show Plotly Code",
    value=st.session_state.show_plotly_code,
    on_change=toggle_state,
    args=("show_plotly_code",),
)
st.sidebar.checkbox(
    "Show Chart",
...
st.sidebar.checkbox(
    ...
    value=st.session_state.show_followup,
    on_change=toggle_state,
    args=("show_followup",),
)
st.sidebar.button(
    "Clear Chat History", on_click=new_convo, use_container_width=True, type="primary"
)
...
if st.session_state.messages[-1]["role"] != "assistant":
    with st.spinner("Thinking..."):
        ...
        # display the sql
        sql, sql_explanation, clarification_request = generate_sql_cached(
            question=my_question, explain=True
        )
        if sql:
            if st.session_state.get("show_sql", True):
                assistant_message_sql = st.chat_message("assistant", avatar=ai_icon)
                    ...
                    if st.session_state.get("show_chart", True):
                        ...
                        if fig is not None:
                            ...
                            st.session_state.messages.append(
                                {
                                    "role": "assistant",
                                    "content": "I couldn't generate a chart",
                                    "avatar": ai_icon,
                                    "content_type": "error",
                                }
                            )
                ...
                if len(followup_questions) > 0:
                    ...
                    for question in followup_questions[:5]:
                        assistant_message_followup.button(
                            question, on_click=set_user_question, args=(question,)
                        )
        else:
